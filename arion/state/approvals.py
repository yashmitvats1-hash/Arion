...
class ApprovalStore(Protocol):
    ...
    def update_request(self, request: ApprovalRequest) -> None:
        """Refresh non-authoritative summary metadata without changing status
        or decision provenance."""
        ...
    def transition_request(
        self, request: ApprovalRequest, expected_status: ApprovalStatus,
    ) -> bool:
        """Conditionally close PENDING as DENIED or EXPIRED only."""
        ...
    def commit_approval_decision(
        self, request: ApprovalRequest, task: Any,
