...
class MemoryStore(Protocol):
    ...
    def record_episode(self, episode: Episode) -> Episode | None:
        """Persist an episode; returns the CANONICAL durable episode for the
        task (one episode per task; first writer wins identity; a re-record
        refreshes content without clobbering the durable reflection link).
        Implementations may return None when they cannot determine the
        canonical row - callers must tolerate None."""
        ...
    ...
    def record_reflection(self, reflection: Reflection) -> Reflection | None:
        """Persist a reflection under the ONE-REFLECTION-PER-EPISODE
        invariant (durable, cross-process): re-recording the same id
        refreshes content; a new id for an episode that already has a
        reflection loses the claim and the canonical first-writer row is
        returned. Implementations may return None when they cannot
        determine the canonical row - callers must tolerate None."""
        ...
